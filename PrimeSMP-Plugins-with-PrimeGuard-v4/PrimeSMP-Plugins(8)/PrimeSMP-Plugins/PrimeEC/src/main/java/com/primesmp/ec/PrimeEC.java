...
public class PrimeEC extends JavaPlugin implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length > 0) {
            ...
            if (target == null) {
                player.sendMessage(ChatColor.RED + "That player must be online to view their ender chest.");
                return true;
            }
            player.openInventory(target.getEnderChest());
            player.sendMessage(ChatColor.GRAY + "Viewing " + target.getName() + "'s ender chest.");
            return true;
        }
